...
@router.get("/popular", response_model=List[AnimeResponse], response_model_by_alias=False)
...
@router.get("/search", response_model=List[AnimeResponse], response_model_by_alias=False)
...
async def add_favorite(anime_id: int, status: str, db: AsyncSession = Depends(get_db),user_id: int = Depends(get_current_user_id)):
...
@router.get("/favorite", response_model = List[UserFavoriteResponse])
async def favorite_list(user_id: int = Depends(get_current_user_id), db: AsyncSession = Depends(get_db)):
    try:
...
@router.delete("/favorite/{anime_id}")
async def delete_favorite(anime_id: int, db:AsyncSession = Depends(get_db),user_id: int = Depends(get_current_user_id)):
    try:
